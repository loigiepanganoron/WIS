namespace IMS.Reports
{
    using System;
    using System.ComponentModel;
    using System.Drawing;
    using System.Windows.Forms;
    using Telerik.Reporting;
    using Telerik.Reporting.Drawing;
    using IMS.Models;
    using System.Data;
    using System.Collections;
    using IMS.@class;
    using System.Globalization;

    /// <summary>
    /// Summary description for rptplc.
    /// </summary>
    public partial class rptplc : Telerik.Reporting.Report
    {

        private pmisEntities pmis = new pmisEntities();
        private IMSEntities db = new IMSEntities();
        public rptplc( string from ,string  to , string in_out, int itemid)
        {
            //
            // Required for telerik Reporting designer support
            //
            InitializeComponent();
            walayclass a = new walayclass();
            //
            // TODO: Add any constructor code after InitializeComponent call
            //
            epsws.serviceSoapClient r = new epsws.serviceSoapClient();
            //var rec = r.POItems(USER.C_officeID).AsEnumerable().Where(b => b.Field<int>("itemid") == itemid).Field<int>("itemName");
           
            //itemName.Value = iName;
            //MMMM dd, yyyy
            //startd.Value = start.ToString("MM/dd/yyyy", CultureInfo.InvariantCulture);
           // endd.Value = end.ToString("MM/dd/yyyy", CultureInfo.InvariantCulture);

           // var vr = a.readplc(from, to, in_out, itemid);

            objectDataSource1.DataSource = a.readplc(from, to, in_out, itemid);
        }
    }
}
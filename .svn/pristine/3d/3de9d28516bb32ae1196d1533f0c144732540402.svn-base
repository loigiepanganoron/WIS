namespace IMS.Reports
{
    ...
    /// </summary>
    public partial class ris_revise_report : Telerik.Reporting.Report
    {
        public ris_revise_report(string controlno)
        {
         ...
         //   DataTable preparedby = (@"select eid,EmpName,position FROM  [pmis].[dbo].[vwMergeAllEmployee]  where eid = '"+eid+"'").DataSet();
             ...
             {
                 SqlCommand com = new SqlCommand(@"select eid,EmpName,position FROM  [pmis].[dbo].[vwMergeAllEmployee]  where eid = '" + eid + "'", con);
                 con.Open();
             ...
             try
             {
               ...
               // userposition.Value = preparedby.Rows[0]["position"].ToString();
                textBox19.Value = USER.C_Name.ToString();
